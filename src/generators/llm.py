...
조건:
1. 핵심 내용과 키워드를 위주로 **3~5줄** 작성하고, 각 줄은 25자 내외의 한 문장으로 써줘.
2. 기사 맥락이 드러나도록 '누가 무엇을 왜/어떻게 했는지'와 주요 배경(기업·모델·지역·시간 등)을 포함해줘.
3. 예상 영향(산업 변화, 시장 반응, 정책/규제 포인트 등)이나 다음 단계가 있다면 간단히 덧붙여줘.
4. 영문 기사나 제목일 경우 반드시 **한글로 번역**하여 요약할 것.
5. 명사형 종결 어미(~함, ~임)나 음슴체(~하다) 대신 해요체(~해요, ~합니다)나 자연스러운 문장으로 작성.
6. 각 줄을 줄바꿈으로 구분하고 불릿이나 번호는 넣지 말 것.
