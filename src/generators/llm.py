...
3. **숫자·통화 단위 규칙 (★단위 오류 빈번 — 가장 주의해서 검산할 것★)**:
   - 원문의 숫자·통화는 **원문에 적힌 단위 그대로** 먼저 정확히 옮길 것. 임의로 자릿수를 늘리거나 줄이지 말 것.
   - 영어 약식 단위 환산표 (반드시 이대로 변환):
     · K(thousand) = 1천
     · M(million) = 100만
     · B(billion) = 10억   ← **'천억'이 아님! 1 billion = 10억**
     · T(trillion) = 1조
   - 달러 금액 환산 예시 (그대로 따를 것):
     · $60B = **600억 달러** (≠ 6천억 달러)
     · $6B = 60억 달러,  $600M = 6억 달러,  $1.5B = 15억 달러,  $2T = 2조 달러
     → 'billion'을 '천억'으로 적으면 **정확히 10배 틀린 값**이 됨. 변환 후 반드시 한 번 더 검산할 것.
   - 달러→원화 환산은 **보조로만** 괄호 안에 병기 (환율 약 1,400원/달러).
     · 검산법: (억 달러 수) × 1,400 ÷ 10,000 = 조원.  예) 600억 달러 × 1,400 = 84조원 → "$60B = 600억 달러(약 84조원)"
   - 환산값의 자릿수(억/조)가 확실하지 않으면 **환산하지 말고 원문 통화(달러 등) 단위만** 표기할 것. 틀린 환산보다 무환산이 낫다.
   - 원문에 없는 수치를 새로 지어내지 말 것.
...
6. **품질 검증**:
   ...
   - **출력 직전, 모든 통화·숫자 단위가 위 3번 환산표와 맞는지 한 번 더 검산할 것**
